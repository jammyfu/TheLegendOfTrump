...
2. 挥剑击碎陶罐，每个陶罐获得 2 枚翡翠。木箱也可破坏；发条守卫需要三次命中，会先蓄力再攻击，可用盾牌正面格挡或闪避。
...
剑与盾不用时背在角色身后，攻击时剑切换到右手，格挡时盾切换到左手，结束后自动背回。冲刺、跳跃、攻击、闪避和承受格挡消耗体力，停止消耗后恢复。
...
测试包含无瞬移的完整草坪路线验证；浏览器用开发接口定位部分测试场景，再以真实键鼠和多点触控操作验证。
